...
function FeatureCardPartner({ href, image }: Card) {
  return (
    <div class="feature-card partner px-[30px] group group-hover:-translate-y-3 w-full max-md:w-full max-w-[327px] relative bg-[#F5F7F9] max-h-[289px] h-[289px] justify-center items-center">
      {
        /* {icon && (
        ...
        <div class="features flex-col w-full gap-0 max-w-[100%]">
          <div class="content-top flex flex-row w-full justify-between max-md:flex-col-reverse">
            ...
            <div class="top-right max-w-[550px] w-[47%] max-md:w-full">
              <div class="text-sm text-[#fc6001] font-[500] section-content pb-6">
                <span>AGRADECIMENTOS ESPECIAIS PARA</span>
              </div>
              <div class="text_link flex flex-row justify-between items-center w-full">
                {title && (
                  <h2 class="font-medium text-[25px] lg:text-[38px] leading-[100%] text-center max-w-4xl z-10">
                    {title}
                  </h2>
                )}
              </div>
          ...
          <div class="bottom-four flex flex-row w-full relative max-md:flex-wrap max-md:grid max-md:grid-cols-2">
            ...
            {cards?.slice(2, 6).map((card) => <FeatureCardPartner {...card} />)}
